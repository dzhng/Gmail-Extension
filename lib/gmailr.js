...
(function($, window) {
    ...
    var Gmailr = {
        ...
        */
		...
		insertReplyButton: function(text) {
			if(!this.loaded) throw "Call to insertReplyButton before Gmail has loaded";
			...
			$(this.elements.canvas.find('.dW .Cq')[0]).append(el);
		...
		insertComposeButton: function(text) {
			if(!this.loaded) throw "Call to insertComposeButton before Gmail has loaded";
			...
			var p = this.elements.canvas.find('.aeH .eh .dW .Cq');
			if(p.length < 1) {
				return null;
			} else {
		...
		insertConversationButton: function(text) {
			if(!this.loaded) throw "Call to insertConversationButton before Gmail has loaded";
			...
			this.elements.canvas.find('.iH').children().append(el);
		...
		insertTopButton: function(text) {
			if(!this.loaded) throw "Call to insertTopButton before Gmail has loaded";
			...
			var p = this.elements.canvas.find('.aeH .nH .Cq').children().children();
			if(p.length < 1) {
				return null;
			} else {
...
//                  this.leftMenu = el.find('.no .nM .TK').first().closest('.nn');
                ...
                if(m && m[1]) {
                    ...
                    switch(action) {
                        ...
                        case "tr": // Deleting
                            ...
                            break;
                        case "sm": // Composing
							// get the email composed
							var text = this.elements.canvas.find('.editable').contents().find('.editable').html();

                            if(this.currentView() == 'compose') {
                                p("User sent an email.");
                                this.executeObQueues('send', text);
                            } else {
                                p("User replied to an email.");
                                this.executeObQueues('reply', text);
                            }
                            break;
                        case "sp": // Spam
        ...
        detectDOMEvents: function(e) {
			...
			var sel = this.elements.canvas.find('.mC .mG');
			if(sel.length > 0) {	// look for the 'reply' button header
				switch($(sel).text()) {
				case 'Reply':
					if(!(this.viewState == 'reply')) {
						this.viewState = 'reply';
						this.executeObQueues('viewChanged', this.viewState);
					}
					break;
				case 'Forward':
					if(!(this.viewState == 'forward')) {
						this.viewState = 'forward';
						this.executeObQueues('viewChanged', this.viewState);
					}
					break;
				case 'Reply to all':
			...
			} else if(this.elements.canvas.find('.M9').length > 0) {	// look for the edit box
				if(!(this.viewState == 'compose')) {
					this.viewState = 'compose';
					this.executeObQueues('viewChanged', this.viewState);
				}
			} else if(this.elements.canvas.find('.ha').length > 0) {	// look for top conversation header
                if(!(this.viewState == 'conversation')) {
                    this.viewState = 'conversation';
					this.executeObQueues('viewChanged', this.viewState);
                }
            } else {
                if(!(this.viewState == 'threaded')) {
					this.viewState = 'threaded';
					this.executeObQueues('viewChanged', this.viewState);
                }
            }
